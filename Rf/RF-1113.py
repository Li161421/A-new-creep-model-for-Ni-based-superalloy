# -*- coding: utf-8 -*-
"""
Created on Mon Nov 13 11:02:49 2023
...
#将数据分为训练和测试集
x = df.iloc[0:186913,[3,4,8,9]]#15和16列是目标性能，其余的14列均为参数
...
#反归一化
Y_returned1 = predictions* (y.max(axis=0) - y.min(axis=0)) + y.min(axis=0)
Y_returned1
Y_returned2 = test_y * (y.max(axis=0) - y.min(axis=0)) + y.min(axis=0)
Y_returned2
print(Y_returned1,Y_returned2)
#反归一化
#进行贝叶斯优化调参
...
rf = RandomForestRegressor(max_depth =30, max_features = 0.77,
                               min_samples_leaf =6,  n_estimators = 171)
rf = rf.fit(train_x, train_y)
score = rf.score(test_x, test_y)
