...
df = pd.read_csv("特征参数数据1.csv",encoding="gbk")
#将数据分为训练和测试集
x = df.iloc[0:186913,0:15]#17列是目标性能，其余的16列均为参数
y = df.iloc[0:186913,15]          #输出17列为目标性能
...
print("train r2:%.3f"%r2_score(train_y,rf.predict(train_x)),"mae:%.7f"%mean_absolute_error(train_y, rf.predict(train_x)),"mse:%.7f"%mean_squared_error(train_y, rf.predict(train_x)))
print("test r2:%.3f"%r2_score(test_y,predictions),"mae:%.7f"%mean_absolute_error(test_y, predictions),"mse:%.7f"%mean_squared_error(test_y, predictions))
