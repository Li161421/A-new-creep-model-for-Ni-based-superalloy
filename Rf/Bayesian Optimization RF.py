...
df = pd.read_csv("Feature_calc31.csv",encoding="gbk")
#将数据分为训练和测试集
x= df.iloc[0:138,28:50]        #输入data_washed31后pearson筛选后的22个参数
y = df.iloc[0:138,26]          #输出25、26列为目标性能
...
from bayes_opt import BayesianOptimization    #引入贝叶斯优化算法
...
print("train r2:%.3f"%r2_score(train_y,rf.predict(train_x)),"mae:%.3f"%mean_absolute_error(train_y, rf.predict(train_x)),"mse:%.3f"%mean_squared_error(train_y, rf.predict(train_x)))
print("test r2:%.3f"%r2_score(test_y,predictions),"mae:%.3f"%mean_absolute_error(test_y, predictions),"mse:%.3f"%mean_squared_error(test_y, predictions))
...
#使用max得到最优
# 1000℃最大强度：'max_depth': 21, 'max_features': 0.12, 'min_samples_split': 6, 'n_estimators': 243
# 室温塑性：'max_depth': 11.960041923435444, 'max_features': 0.1, 'min_samples_split': 3.3405557328847757, 'n_estimators': 172.31102952518452
